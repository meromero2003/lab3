...
const getInserterItems = exports.getInserterItems = (0, _data.createRegistrySelector)(select => (0, _data.createSelector)((state, rootClientId = null, options = {}) => {
  ...
  if (options[_utils.withRootClientIdOptionKey]) {
    blockTypeInserterItems = blockTypeInserterItems.reduce((accumulator, item) => {
      ...
      while (!canInsertBlockTypeUnmemoized(state, item.name, item.rootClientId)) {
        if (!item.rootClientId) {
          ...
          try {
            sectionRootClientId = (0, _lockUnlock.unlock)(getSettings(state)).sectionRootClientId;
          } catch (e) {}
...
const __experimentalGetParsedPattern = exports.__experimentalGetParsedPattern = (0, _data.createRegistrySelector)(select => (0, _data.createSelector)((state, patternName) => {
...
}, (state, patternName) => [(0, _lockUnlock.unlock)(select(_constants.STORE_NAME)).getPatternBySlug(patternName)]));
...
const __experimentalGetAllowedPatterns = exports.__experimentalGetAllowedPatterns = (0, _data.createRegistrySelector)(select => {
  return (0, _data.createSelector)((state, rootClientId = null) => {
    const {
      getAllPatterns,
    ...
    const availableParsedPatterns = parsedPatterns.filter(({
      blocks
    }) => (0, _utils.checkAllowListRecursive)(blocks, allowedBlockTypes));
...
function __unstableHasActiveBlockOverlayActive(state, clientId) {
  ...
  if (editorMode === 'zoom-out') {
    const {
      sectionRootClientId
    } = (0, _lockUnlock.unlock)(getSettings(state));
